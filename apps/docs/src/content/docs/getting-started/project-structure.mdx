...
Velox TS projects use a monorepo structure with separate apps for backend (`apps/api`) and frontend (`apps/web`).
...
`procedures/` is also the canonical [HMR boundary](/docs/cli/dev-server/#configure-hmr-boundaries) — `velox dev` (HMR is on by default) reloads procedure files in-process on change without restarting the server, since procedure handlers are pure functions over `(input, ctx)` and don't hold long-lived state.
...
- [Procedures](/docs/endpoints/procedures/) - Define your API
