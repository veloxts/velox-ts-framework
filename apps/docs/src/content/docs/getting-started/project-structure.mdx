...
`procedures/` is also the canonical [HMR boundary](/docs/cli/dev-server/#configure-hmr-boundaries) — `velox dev --hmr` reloads procedure files in-process on change without restarting the server, since procedure handlers are pure functions over `(input, ctx)` and don't hold long-lived state.
