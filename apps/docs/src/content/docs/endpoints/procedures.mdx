...
The `.output()` method also accepts resource schemas for context-dependent output projection. The Resource API lets you define field visibility levels (`public`, `authenticated`, `admin`) once and project data based on access level.
...
getProfile: procedure()
  ...
  .output(UserSchema) // auto-projects { id, name, email }
  .query(async ({ input, ctx }) => {
