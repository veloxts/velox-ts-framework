...
### `.expose(schema)` with Resource Schemas

The `.expose()` method accepts resource schemas for context-dependent output projection. The Resource API lets you define field visibility levels (`public`, `authenticated`, `admin`) once and project data based on access level.
...
getProfile: procedure()
  ...
  .expose(UserSchema) // auto-projects { id, name, email }
  .query(async ({ input, ctx }) => {
...
procedure()
  ...
  .output(schema)      // 3. Output schema or resource schema (optional)
  .expose(schema)      // 3b. Context-dependent output projection (optional)
...
- [Architectural Patterns](/docs/advanced/architectural-patterns/) - Scaling procedures and plugin isolation
