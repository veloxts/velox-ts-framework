...
Use VeloxTS [error classes](/docs/core/error-handling/) to return meaningful HTTP responses. The framework provides built-in error classes for common HTTP status codes:
...
Use the `db()` wrapper from `@veloxts/orm` to automatically convert Prisma errors into appropriate VeloxTS error classes:
...
<Aside type="note" title="Raw Body for Signature Verification">
Webhook signature verification (Stripe, GitHub, etc.) requires the raw request body as a Buffer. Register the `rawBodyPlugin` from `@veloxts/core` to make `request.rawBody` available for HMAC verification.
</Aside>
