...
When a guard already determines the access level, repeating it in `.expose()` is redundant. Chain `.guardNarrow()` with `.expose()` on the procedure builder and the executor projects fields automatically:
...
3. **Use guards for automatic projection** — When access level maps directly to a guard, use `.guardNarrow()` + `.expose()` to eliminate manual projection calls entirely.
...
- [Procedures](/docs/endpoints/procedures/) — builder methods (`.expose()`, `.guardNarrow()`)
