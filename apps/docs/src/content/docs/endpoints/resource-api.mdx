...
3. **Use guards for automatic projection** — When access level maps directly to a guard, use `.guardNarrow()` + `.output()` to eliminate manual projection calls entirely.
...
- [Procedures](/docs/endpoints/procedures/) — builder methods (`.output()`, `.guardNarrow()`)
