...
When a guard already determines the access level, repeating it in `.output()` is redundant. Chain `.guardNarrow()` with `.output()` on the procedure builder and the executor projects fields automatically:
