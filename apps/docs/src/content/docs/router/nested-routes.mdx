...
Your input schema MUST include the parent parameter. Velox TS extracts it from the URL and passes it to your handler through the `input` object.
...
If you omit the `param` field, Velox TS uses the same singularization logic:
