...
When you start with VeloxTS, the procedure pattern gets you running fast: type-safe endpoints, auto-generated REST routes, and integrated auth — all with minimal configuration.
...
Traditional enterprise architectures enforce strict separation of concerns from day one — Controllers, DTOs, Services, Repositories — regardless of domain complexity. VeloxTS takes a different approach: it compresses this entire flow into a single, type-inferred **Procedure Definition** to eliminate boilerplate without sacrificing compiler safety.
...
Because VeloxTS relies on compiler-driven type inference rather than code generation, you can structure domain directories however you like without breaking client type safety.
...
Scaling effectively requires strict access control over shared dependencies and middleware. VeloxTS provides two complementary isolation mechanisms.
...
const requireStripeCustomer = async ({ ctx, next }: { ctx: BaseContext; next: () => Promise<unknown> }) => {
  if (!ctx.user) throw new Error('Unauthorized');
  // Extend context directly — this is how VeloxTS middleware works
