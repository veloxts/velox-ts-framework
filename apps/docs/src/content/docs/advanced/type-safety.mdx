...
Velox TS provides complete end-to-end type safety from database to frontend without any code generation step. Types flow naturally through the TypeScript compiler.

## The Velox TS Type Philosophy
...
**Velox TS approach:** Types are inferred instantly through TypeScript's type system. No build step, no generated files—you get autocomplete and error feedback as you type.
...
```typescript
// procedures/users.ts
import { procedures, procedure, resourceSchema } from '@veloxts/velox';
...
export const userProcedures = procedures('users', {
  // Tagged resource view - guard + .output() determines output type
  getUser: procedure()
    ...
    .query(async ({ input, ctx }) => {
      ...
      // Just return data - projection is automatic based on tagged view
  ...
  createUser: procedure()
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Projected to authenticated fields
    }),
});
```
...
Velox TS uses TypeScript declaration merging to extend the context object:
...
.query(async ({ ctx }) => {
  ...
  return resource(user, UserSchema.public); // Runtime error - name is undefined
})

// Correct - fetch all required fields
.query(async ({ ctx }) => {
  const user = await ctx.db.user.findUnique({ where: { id } });
  return resource(user, UserSchema.public); // Works - all fields present
})
```

## Related Content

- [Procedures](/docs/endpoints/procedures/) - Define type-safe endpoints
