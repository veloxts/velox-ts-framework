...
Server actions let you call server-side functions directly from React components — with form handling, validation, and error reporting built in. Velox TS adds the `validated()` wrapper for Zod schema validation and typed return values.
