...
<Aside type="note">
This section covers RSC templates (`--rsc`, `--rsc-auth`). For SPA templates that consume Velox TS APIs from a separate frontend, see [Client Package](/docs/client/overview/).
</Aside>
...
└─────────────────────────────────────┘
     ↓
Velox TS Procedures (Business Logic)
     ↓
Database (Prisma)
```
...
├── api/
│   ├── procedures/      # Velox TS procedures
