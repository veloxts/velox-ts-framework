...
export const userProcedures = procedures('users', {
  // Authenticated endpoint - auto-projects { id, name, avatarUrl, email, createdAt }
...
**How it works:**
1. The `guardNarrow()` accepts guards with an `accessLevel` property
2. After the guard passes, the executor tracks the access level
3. When the handler returns, fields are automatically projected
4. No manual `.forX()` calls needed - output types are inferred

### Manual Projection (Complex Cases)

For public endpoints or conditional logic, use explicit projection methods:

```typescript
import { resource } from '@veloxts/router';

// In a public endpoint - no guard, explicit projection
