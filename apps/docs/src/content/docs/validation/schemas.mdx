...
For defining field-level visibility with `resourceSchema()` and projecting data based on access levels, see the dedicated [Resource API](/docs/endpoints/resource-api/) guide.
...
VeloxTS procedures automatically infer input and output types:
...
When using the [Resource API](/docs/endpoints/resource-api/), output types are narrowed based on the projection level (public, authenticated, or admin).
...
- [Procedures](/docs/endpoints/procedures/) - Using schemas in procedure definitions
