---
title: JWT Authentication
description: Token-based authentication with access and refresh tokens.
---

JWT authentication issues short-lived access tokens and long-lived refresh tokens. Velox TS handles token generation, verification, refresh rotation, and automatic `ctx.user` injection.

## Setup

```typescript
import { jwtManager, authPlugin } from '@veloxts/auth';

const jwt = jwtManager({
  secret: process.env.JWT_SECRET!,
  refreshSecret: process.env.JWT_REFRESH_SECRET!,
  accessTokenExpiry: '15m',
  refreshTokenExpiry: '7d',
});

await app.register(authPlugin, { jwt });
```

## Login Flow

```typescript
login: procedure()
  .input(z.object({
    email: z.string().email(),
    password: z.string(),
  }))
  .mutation(async ({ input, ctx }) => {
    const user = await ctx.db.user.findUnique({
      where: { email: input.email },
    });

    if (!user || !await verifyPassword(input.password, user.passwordHash)) {
      throw new Error('Invalid credentials');
    }

    const tokens = await ctx.jwt.generateTokenPair({
      sub: user.id,
      email: user.email,
    });

    return { user, ...tokens };
  }),
```

## Token Refresh

```typescript
refreshToken: procedure()
  .input(z.object({ refreshToken: z.string() }))
  .mutation(({ input, ctx }) => ctx.jwt.refreshTokens(input.refreshToken)),
```

## Protecting Routes

```typescript
import { authenticated } from '@veloxts/auth';

getProfile: procedure()
  .guard(authenticated)
  .query(({ ctx }) => ctx.user),
```

## Client Usage

```typescript
// Store tokens
localStorage.setItem('accessToken', tokens.accessToken);
localStorage.setItem('refreshToken', tokens.refreshToken);

// Include in requests
fetch('/api/profile', {
  headers: {
    Authorization: `Bearer ${accessToken}`,
  },
});
```

## Related Content

- [Guards](/docs/authentication/guards/) - Authorization
- [Sessions](/docs/authentication/sessions/) - Alternative approach
