...
<Aside type="tip" title="Related primitives">
For role-based **field projection** on responses (returning a public profile to anonymous visitors and a richer profile to authenticated users), see [Resource API](/docs/endpoints/resource-api/) — the `public`, `authenticated`, and `admin` visibility tiers align cleanly with guard levels.

For authorization that needs the parsed `input` AND middleware-extended `ctx` (e.g. resource-ownership checks where the resource is loaded by `.use()`), see [Post-Middleware Checks](/docs/endpoints/post-middleware-checks/) — `.check()` is the right primitive when `.guard()` runs too early.
</Aside>
