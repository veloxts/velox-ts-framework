...
If you need `@trpc/react-query` specifically (for existing codebases or specific tRPC features), use the `AsTRPCRouter` type transformer:
...
// Transform VeloxTS router type to tRPC-compatible type
export const trpc = createTRPCReact<AsTRPCRouter<AppRouter>>();

// Now works with @trpc/react-query
const users = trpc.users.list.useQuery();
```

This is an escape hatch for specific needs. For most VeloxTS apps, `@veloxts/client` is the recommended approach.
