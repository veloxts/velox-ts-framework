...
The full-stack RSC architecture runs your entire application — API, server rendering, and client interactivity — in a single codebase using React Server Components and Vinxi. Server components fetch data directly without API calls, and client components hydrate only where interactivity is needed. Choose this for web applications where you want unified deployment, streaming SSR, and zero client-server boilerplate.
...
Velox TS uses **Vinxi** as the HTTP handler, with Fastify embedded for API routes:
