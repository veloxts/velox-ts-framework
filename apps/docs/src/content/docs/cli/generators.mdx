...
<Aside type="tip" title="Multiple models? Use velox sync">
If you have multiple Prisma models and want schemas + procedures for all of them at once, use **[`velox sync`](/docs/cli/sync/)** instead. `velox make` is best for adding a single new model to an existing project.
</Aside>
...
Use **`namespace`** when you already have a Prisma model and need procedures for it. Use **`resource`** when starting from scratch — it creates the Prisma model, schema, procedures, and tests all at once.
</Aside>
