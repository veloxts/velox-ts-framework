...
<Aside type="tip" title="Already scaffolded?">
If you created your project with `create-velox-app`, Prisma 7 is already configured. You can skip to [Velox TS Integration](#velox-ts-integration) or [Common Errors](#common-errors) if you run into issues.
</Aside>

## How Prisma 7 Works

Prisma 7 uses **driver adapters** to connect to databases at runtime. Three configuration files work together:

- **`prisma.config.ts`** — Tells the Prisma CLI where to find the schema and database URL
- **`prisma/schema.prisma`** — Defines your data model (no connection URL here)
- **`src/config/database.ts`** — Creates the runtime client with a driver adapter
...
```typescript
// database.ts — runtime client with adapter
...
This separation gives you full control over connection pooling, timeouts, and enables edge/serverless deployments.
...
**Cause:** `datasourceUrl` is not a valid PrismaClient option in Prisma 7.
...
**Cause:** `url` field present in `schema.prisma` datasource block.
...
## Migrating from Prisma 5/6

<Aside type="note">
This section is for developers bringing an existing Prisma project into Velox TS. If you started with `create-velox-app`, you can skip this.
</Aside>
