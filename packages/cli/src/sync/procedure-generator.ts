...
import { toKebabCase } from '../generators/utils/naming.js';
...
function generateGetProcedure(plan: ProcedureFilePlan, names: NamesRef): string {
	...
	if (plan.outputStrategy === 'output') {
		const schemaName = hasIncludes
			? `${names.pascal}WithRelationsSchema`
			: `${names.pascal}Schema`;
...
function generateDeleteProcedure(plan: ProcedureFilePlan, names: NamesRef): string {
	...
	if (plan.model.isJoinTable && plan.model.uniqueConstraints.length > 0) {
		...
		const inputFields = constraintFields
			.map((f) => `${f}: z.string().uuid()`)
			.join(', ');
		const whereFields = constraintFields
			.map((f) => `${f}: input.${f}`)
			.join(', ');
