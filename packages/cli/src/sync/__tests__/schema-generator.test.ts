...
function makePlan(overrides: Partial<SchemaFilePlan> & { model: SyncModelInfo }): SchemaFilePlan {
  ...
  const inputFields =
    overrides.fields ?? [...model.fields].filter((f) => !f.isAutoManaged && !f.isSensitive);
  return {
...
describe('generateSchemaFile', () => {
  ...
  it('includes non-user foreign key fields in CreateSchema', () => {
    const model: SyncModelInfo = {
      name: 'Comment',
      fields: [idField, contentField, categoryIdField, authorIdField, createdAtField],
      relations: [],
      isJoinTable: false,
      hasTimestamps: true,
      uniqueConstraints: [],
