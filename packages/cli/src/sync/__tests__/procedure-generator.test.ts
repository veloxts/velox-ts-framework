...
import type { CrudChoices, ProcedureFilePlan, SyncFieldInfo, SyncModelInfo, SyncRelationInfo } from '../types.js';
...
function makePlan(overrides: Partial<ProcedureFilePlan> & { model: SyncModelInfo }): ProcedureFilePlan {
	return {
...
describe('generateProcedureFile', () => {
	...
	it('uses resource() and resourceCollection() for resource strategy', () => {
		...
		expect(output).toContain("import { procedure, procedures, resource, resourceCollection } from '@veloxts/router';");
	});
